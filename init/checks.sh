...
_checkDefaultVars() {
    ...
    declare -rA def_vals=(
        ...
        [UPSTREAM_REPO]="https://github.com/AftahBagas/AlphaZ-Plugins"
...
_checkUnoffPlugins() {
    ...
    if test $LOAD_UNOFFICIAL_PLUGINS = true; then
        ...
        gitClone --depth=1 https://github.com/AftahBagas/Alpha-Plugins.git
        editLastMessage "\t\tUpgrading PIP ..."
        upgradePip
        editLastMessage "\t\tInstalling Requirements ..."
        installReq Alpha-Plugins
        editLastMessage "\t\tCleaning ..."
        rm -rf userge/plugins/unofficial/
        mv Alpha-Plugins/plugins/ userge/plugins/unofficial/
        cp -r Alpha-Plugins/resources/* resources/
        rm -rf Alpha-Plugins/
        deleteLastMessage
