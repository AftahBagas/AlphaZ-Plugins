...
class Alphaz(_AbstractUserge):
    ...
    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting AlphaZ Plugins Configs")
        kwargs = {
        ...
        if Config.HU_STRING_SESSION and Config.BOT_TOKEN:
            ...
            kwargs['bot'] = AlphazBot(bot=self, **kwargs)
    ...
    async def start(self) -> None:
        ...
        _LOG.info(_LOG_STR, "Starting Alphaz")
        await super().start()
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Starting AlphazBot")
    ...
    async def stop(self) -> None:  # pylint: disable=arguments-differ
        """ stop client and bot """
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Stopping AlphazBot")
            await self._bot.stop()
        _LOG.info(_LOG_STR, "Stopping Alphaz")
        await super().stop()
        _close_db()
    ...
    def begin(self, coro: Optional[Awaitable[Any]] = None) -> None:
        ...
        async def _shutdown(sig: signal.Signals) -> None:
            _LOG.info(_LOG_STR, f"Received Stop Signal [{sig.name}], Exiting AlphaZ Plugins ...")
            await _finalize()
        ...
        logbot.edit_last_msg("AlphaZ Plugins has Started Successfully !")
        logbot.end()
