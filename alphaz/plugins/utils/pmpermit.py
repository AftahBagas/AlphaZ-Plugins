...
from alphaz import userge, filters, Message, Config, get_collection
from alphaz.utils import SafeDict
...
@alphaz.on_filters(~allowAllFilter & filters.incoming & filters.private & ~filters.bot
                   & ~filters.me & ~filters.service & ~Config.ALLOWED_CHATS, allow_via_bot=False)
async def uninvitedPmHandler(message: Message):
    """ pm message handler """
    user_dict = await alphaz.get_user_dict(message.from_user.id)
    ...
    if message.from_user.id in pmCounter:
        ...
        if alphaz.has_bot and _IS_INLINE:
            try:
                bot_username = (await userge.bot.get_me()).username
                k = await alphaz.get_inline_bot_results(bot_username, "pmpermit")
...
@alphaz.on_filters(~allowAllFilter & filters.outgoing & ~filters.edited
...
async def outgoing_auto_approve(message: Message):
    ...
    await ALLOWED_COLLECTION.update_one(
        {'_id': userID}, {"$set": {'status': 'allowed'}}, upsert=True)
    user_dict = await alphaz.get_user_dict(userID)
    await CHANNEL.log(f"**#AUTO_APPROVED**\n{user_dict['mention']}")

if alphaz.has_bot:
    @alphaz.bot.on_callback_query(filters.regex(pattern=r"pm_allow\((.+?)\)"))
    async def pm_callback_allow(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            await alphaz.unblock_user(userID)
            user = await alphaz.get_users(userID)
            if userID in Config.ALLOWED_CHATS:
    ...
    @alphaz.bot.on_callback_query(filters.regex(pattern=r"pm_block\((.+?)\)"))
    async def pm_callback_block(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            await alphaz.block_user(userID)
            if userID in pmCounter:
                del pmCounter[userID]
            if userID in Config.ALLOWED_CHATS:
                Config.ALLOWED_CHATS.remove(userID)
    ...
    @alphaz.bot.on_callback_query(filters.regex(pattern=r"^pm_spam$"))
    async def pm_spam_callback(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            user_dict = await alphaz.get_user_dict(c_q.from_user.id)
            await c_q.edit_message_text(
                blocked_message.format_map(SafeDict(**user_dict)))
            await alphaz.block_user(c_q.from_user.id)
            await asyncio.sleep(1)
            await CHANNEL.log(
                f"#BLOCKED\n{c_q.from_user.mention} has been blocked due to spamming in pm !! ")

    @alphaz.bot.on_callback_query(filters.regex(pattern=r"^pm_contact$"))
